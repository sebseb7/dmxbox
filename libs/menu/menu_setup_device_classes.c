...
void menu_setup_device_classes()
{
	...
	if(redraw)
	{
		...
		if((get_device_class_count()<10)&&(active_row==0))
		{
	...
	if(check_button_press(&x,&y)==1)
	{
		...
		if(field == 2)
		{
			...
			{
				set_current_execution(menu_setup_device_classes_edit);
				set_device_class_to_edit(active_row-1);
			}
		}
		if(field == 3)
		{
