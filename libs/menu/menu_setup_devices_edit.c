...
void set_device_to_edit(uint8_t idx)
{
	device_to_edit=idx;
}

void menu_setup_devices_edit()
{
	if(init_device)
	{
		new_device=get_device(device_to_edit);
		init_device=0;
	...
	if(redraw)
	{
		...
		uint16_t text_width =  get_text_width_16pt("Edit Device");

		draw_text_16pt((LCD_WIDTH-text_width)>>1,9, "Edit Device", 200,200,255);
