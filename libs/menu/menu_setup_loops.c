...
void menu_setup_loops()
{
	...
	if(redraw)
	{
		...
		uint16_t text_width =  get_text_width_16pt("Setup Loops");

		draw_text_16pt((LCD_WIDTH-text_width)>>1,9, "Setup Loops", 200,200,255);
		...
		char buf2[30];
		draw_lineitem("","<new>");
		for(int i = 0; i < get_loop_count();i++)
		{
			snprintf(buf,30,"%i",i);
			snprintf(buf2,30,"%s",get_loop(i)->name);
		...
		if((get_loop_count()<10)&&(active_row==0))
		{
	...
	if(check_button_press(&x,&y)==1)
	{
		...
		if(field == 2)
		{
			...
			if(active_row==0)
			{
				if(get_loop_count()<50)
				{
					set_current_execution(menu_setup_loops_add);
				}
			}
			else
			{
